...
class GUI:
    ...
    model = None
    MODEL_PATH = "weights/triplet_models_2019_1_221_99.h5"
    x_train = None
    y_train = None
    data_frame = None
    DATA_FRAME_PATH = "dataframe_2019_221.zip"
    size = (221, 221)
    embs = None
    EMBS_PATH = "embs2019.npy"
    X_PATH = "x_train_221_2019.npy"
    Y_PATH = "y_train_221_2019.npy"
    hog = None
    is_saving = False
    ...
    def detect(self, image):
        ...
        # faces = self.hog.detect_faces(image)
        end = time.time()
        if len(faces) == 0:
            print("NOT DETECTED")
    ...
    def load_data(self):
        ...
        try:
            if os.path.exists(self.MODEL_PATH):
                ...
                #                                                nesterov=True),
                #                    loss=vgg_model._loss_tensor)
                self.model.compile(optimizer=tf.optimizers.SGD(lr=0.01,
                                                               decay=0.001,
                                                               momentum=0.9,
                                                               nesterov=True),
                                   loss=vgg_model._loss_tensor)
    ...
    def predict_image(self, image, faces):
        ...
        for face in faces:
            ...
            if minimum > self.THRESH_HOLD:
                ...
                print(f"EMB: {emb}")
                cv2.rectangle(image, (x, y), (x + w, y + h), (0, 255, 0), 2)
                cv2.putText(image, str(name), (x, y + h + 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                cv2.putText(image, str(round(emb, 6)), (x, y + h + 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
